...
function listarPropostas($pedido){
    ...
    tv.nome AS veiculo, v.placa, m.email, m.avaliacao, m.telefone, m.pagamentos AS pagamento
    FROM proposta pr
...
function avaliacao($motorista){
    $conexao = obterConexao();
    $sql = "SELECT  email_m,
            COUNT(avaliacao) AS quantidade_de_avaliacoes,
    ...
    $stmt->close();
    $sql = "UPDATE motorista SET avaliacao = ? WHERE email = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("ss",$avaliacoes["media"],$motorista);
    $stmt->execute();
    $stmt->close();
    $conexao->close();
}
