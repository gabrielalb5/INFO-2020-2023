...
function listarPedidos_conf_cliente(){
    ...
    INNER JOIN categoria_pedido cp ON p.categoria = cp.id_cp
    WHERE cliente = ? AND status = 'Confirmado' OR status = 'Iniciado' ORDER BY p.data_entrega";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $resultado = $stmt->get_result();
...
function listarPedidos_conf_motorista(){
    ...
    INNER JOIN categoria_pedido cp ON p.categoria = cp.id_cp
    WHERE motorista = ? AND status = 'Confirmado' OR status = 'Iniciado' ORDER BY p.data_entrega";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $resultado = $stmt->get_result();
...
function proximaEntrega(){
    $conexao = obterConexao();
    $email = $_SESSION["logado"];
    $sql = "SELECT id_p, data_entrega, horario
    FROM pedido p
