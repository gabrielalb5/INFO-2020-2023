...
function cadastrarMotorista(){
    ...
    if (mysqli_num_rows($resultado)>0){
        ...
        if(mysqli_num_rows($resultado)>0){
            ...
            $sql = "INSERT INTO motorista (nome, sobrenome, email, senha, cpf, telefone, uf, cidade)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conexao->prepare($sql);
            $stmt->bind_param("ssssssss", $motorista["nome"], $motorista["sobrenome"], $motorista["email"], $senha_md5, $motorista["cpf"], $motorista["telefone"], $motorista["uf"], $motorista["cidade"]);
            $stmt->execute();
            $stmt->close();
            $_SESSION["h2"] = "Bem-vindo ao time!";
            $_SESSION["p"] = 'Acesse seu perfil em "Entrar"';
...
function updateMotorista($nome,$sobrenome,$senha,$cpf,$telefone,$uf,$cidade){
    $email = $_SESSION["logado"];
    $conexao = obterConexao();
    $senha_md5 = md5($senha);
    $sql = "UPDATE motorista SET nome = ?, sobrenome = ?, senha = ?, cpf = ?, telefone = ?, uf = ?, cidade = ? WHERE email = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("ssssssss",$nome,$sobrenome,$senha_md5,$cpf,$telefone,$uf,$cidade,$email);
    $stmt->execute();
    if($stmt->affected_rows>0){
...
function updateVeiculo($tipo,$renavam,$marca,$modelo,$placa,$ano,$cor){
    ...
    $stmt->execute();
    if($stmt->affected_rows>0){
        $_SESSION["msg"] = 'Dados do veículo alterados com sucesso!';
        $_SESSION["tipo_msg"] = "alert-warning";
    }
    $stmt->close();
    $conexao->close();
...
function listarPedidos_cliente(){
    ...
    $email = $_SESSION["logado"];
    $lista_pedidos = [];
    $sql = "SELECT p.descricao, p.origem, p.destino, p.data_entrega, p.horario, p.valor, cp.nome as 'categoria' FROM pedido p
...
function listarPedidos_motorista($cidade){
    $conexao = obterConexao();
    $lista_pedidos = [];
    $sql = "SELECT p.descricao, p.origem, p.destino, p.data_entrega, p.horario, p.valor, p.status, cp.nome as 'categoria' FROM pedido p
    INNER JOIN categoria_pedido cp ON p.categoria = cp.id_cp
    WHERE p.origem LIKE '%$cidade%'";
    $stmt = $conexao->prepare($sql);
    $stmt->execute();
    $resultado = $stmt->get_result();
    while($pedido = mysqli_fetch_assoc($resultado)){
