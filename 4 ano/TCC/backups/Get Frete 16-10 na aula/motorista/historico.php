...
$registros = $con->query("SELECT COUNT(id_h) count from historico WHERE email_m = '$email'")->fetch()["count"];
...
if (empty($lista_historico)) {
    echo '<h4>Histórico</h4><p class="text-secondary">Você não possui entregas finalizadas</p>';
} else {
    ?>
