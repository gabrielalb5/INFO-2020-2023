...
function arquivarHistorico($pedido,$avaliacao){
    ...
    //var_dump($p);
    //die();
    $sql = "INSERT INTO historico (id_pedido,data_pedido,finalizado,descricao,origem,destino,data_entrega,horario,ajudante,valor,cliente,cpf_c,email_c,motorista,cpf_m,email_m,veiculo,avaliacao)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("issssssssssssssssi",$p["id_p"],$p["data_pedido"],$p["finalizado"],$p["descricao"],$p["origem"],$p["destino"],$p["data_entrega"],$p["horario"],$p["ajudante"],$p["valor"],$p["cliente"],$p["cpf_c"],$p["email_c"],$p["motorista"],$p["cpf_m"],$p["email_m"],$p["veiculo"],$avaliacao);
    $stmt->execute();
    $stmt->close();
    $sql = "DELETE FROM pedido WHERE id_p = ?";
    $stmt = $conexao->prepare($sql);
...
function historico(){
    ...
    $conexao = obterConexao();
    if($perfil=="motorista"){
        $sql = "SELECT id_h, id_pedido, data_pedido, finalizado,
        descricao, origem, destino, data_entrega, horario, valor,
        cliente, cpf_c, email_c, motorista, cpf_m, email_m,
        veiculo, ajudante, avaliacao
        FROM historico WHERE email_m = ? ORDER BY data_entrega DESC";
    }else{
        $sql = "SELECT id_h, id_pedido, data_pedido, finalizado,
        descricao, origem, destino, data_entrega, horario, valor,
        cliente, cpf_c, email_c, motorista, cpf_m, email_m,
        veiculo, ajudante, avaliacao
        FROM historico h WHERE email_c = ? ORDER BY data_entrega DESC";
    }
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("s",$email);
    $stmt->execute();
