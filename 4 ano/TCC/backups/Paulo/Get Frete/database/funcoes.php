...
function escolherProposta($proposta_escolhida){
    $conexao = obterConexao();
    $sql = "SELECT id_pr, pedido, motorista, veiculo, valor FROM proposta WHERE id_pr = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("i",$proposta_escolhida);
    $stmt->execute();
    $resultado = $stmt->get_result();
    $proposta = mysqli_fetch_assoc($resultado);
        $proposta_id = $proposta['id_pr'];
    ...
    findOrderId($motorista);
    $sql = "DELETE FROM notificacao WHERE id_proposta = ? AND tipo = 1";
    $stmt = $conexao->prepare($sql);
...
function listarPedidos_conf_cliente(){
    ...
    p.origem, p.destino, p.data_entrega, p.horario, cp.nome as 'categoria',
...
function arquivarHistorico($pedido,$avaliacao){
    ...
    p.data_entrega, p.horario, REPLACE(p.valor, '.', ',') as valor,
    CONCAT(c.nome, ' ', c.sobrenome) AS cliente, c.cpf as cpf_c, c.email as email_c,
    CONCAT(m.nome, ' ', m.sobrenome) AS motorista, m.cpf as cpf_m, m.email as email_m,
    ...
    //var_dump($p);
    //die();
    $sql = "INSERT INTO historico (id_pedido,data_pedido,finalizado,descricao,origem,destino,data_entrega,horario,valor,cliente,cpf_c,email_c,motorista,cpf_m,email_m,veiculo,avaliacao)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("isssssssssssssssi",$p["id_p"],$p["data_pedido"],$p["finalizado"],$p["descricao"],$p["origem"],$p["destino"],$p["data_entrega"],$p["horario"],$p["valor"],$p["cliente"],$p["cpf_c"],$p["email_c"],$p["motorista"],$p["cpf_m"],$p["email_m"],$p["veiculo"],$avaliacao);
    $stmt->execute();
    $stmt->close();
    $sql = "DELETE FROM pedido WHERE id_p = ?";
    $stmt = $conexao->prepare($sql);
...
function avaliacao(){
    $sql = "SELECT avaliacao, COUNT (*) AS quantidade, avaliacao / COUNT(*) AS valor_dividido
    FROM historico
    GROUP BY avaliacao
    WHERE email_m = ?";
}

// Encontra o Id da PROPOSTA
function findProposalId($email) {
    $conexao = obterConexao();

    $sql = "SELECT pr.id_pr, pr.pedido, p.cliente
    FROM proposta pr
    INNER JOIN pedido p ON pr.pedido = p.id_p
    WHERE pr.motorista = '$email'";
    $result = $conexao->query($sql);
    
    if ($result->num_rows > 0) {
        $proposta = $result->fetch_assoc();

        sendNotification($proposta['id_pr'], $proposta['pedido'], $proposta['cliente'], 1);
    }
    $result->close();
}

// Envia Notificacoes
function sendNotification($id_proposta, $id_pedido, $perfil, $tipo) {
    $conexao = obterConexao();

    $sql = "INSERT INTO notificacao (id_proposta, id_pedido, perfil, tipo) 
    VALUES ('$id_proposta', '$id_pedido', '$perfil', '$tipo')";
    $conexao->query($sql);
    $conexao->close();
}

// Checa Proposta
function checkNotifications($tipo, $perfil) {
    $conexao = obterConexao();
    $sql = "SELECT lido FROM notificacao 
    WHERE lido != 1 AND tipo = $tipo AND perfil = '$perfil'";
    $result = $conexao->query($sql);
...
function countNotifications() {
    $conexao = obterConexao();
    $sql = "SELECT lido FROM notificacao WHERE lido != 1";
    $result = $conexao->query($sql);
...
// Listar notificações
function listNotifications($tipo, $perfil) {
    $notifications = [];
    $conexao = obterConexao();

    if ($tipo != 2) {
        $sql = "SELECT n.id, n.lido, n.id_proposta,
        CONCAT(m.nome, ' ', m.sobrenome) AS motorista,
        m.email, p.id_p, p.descricao, pr.id_pr, pr.pedido
        FROM notificacao n
        INNER JOIN proposta pr ON n.id_proposta = pr.id_pr
        INNER JOIN motorista m ON pr.motorista = m.email
        INNER JOIN pedido p ON pr.pedido = p.id_p 
        WHERE n.tipo = $tipo AND n.perfil = '$perfil'
        ORDER BY n.lido";
    } else {
        $sql = "SELECT n.id, n.lido,
        CONCAT(m.nome, ' ', m.sobrenome) AS motorista,
        m.email, p.id_p, p.descricao
        FROM notificacao n
        INNER JOIN pedido p ON n.id_pedido = p.id_p 
        INNER JOIN motorista m ON p.motorista = m.email
        WHERE n.tipo = $tipo AND n.perfil = '$perfil'
        ORDER BY n.lido";
    ...
    $stmt->execute();
    $resultado = $stmt->get_result();
    while($notification = mysqli_fetch_assoc($resultado)){
...
// Altera campo 'lido' para 1
function readNotification($search) {
    $conexao = obterConexao();
    $sql = "UPDATE notificacao SET lido = 1 WHERE id = $search";
    $conexao->query($sql);
    $conexao->close();
    
    if ($_SESSION['perfil'] == 'motorista') {
        header('Location: ../motorista/perfil.php');
    } else {
...
// Apaga notificações
function deleteNotifications($tipo, $perfil) {
    $conexao = obterConexao();

    $sql = "DELETE FROM notificacao WHERE tipo = $tipo AND perfil = '$perfil'";
    $conexao->query($sql);
    $conexao->close();
...
// Lê notificações
function seeNotifications($tipo, $perfil) {
    $conexao = obterConexao();

    $sql = "UPDATE notificacao SET lido = $tipo AND perfil = '$perfil'";
    $conexao->query($sql);
    $conexao->close();
...
// NOTIFICAÇÕES PARA O MOTORISTA
// Encontra o Id da PROPOSTA ACEITA
function findOrderId($motorista) {
    // id_proposta, id_pedido, perfil, tipo
    $conexao = obterConexao();

    $sql = "SELECT id_p, cliente FROM pedido WHERE motorista = '$motorista'";
    $result = $conexao->query($sql);
    
    if ($result->num_rows > 0) {
        $data = $result->fetch_assoc();

        sendNotification(-1, $data['id_p'], $motorista, 2);
    }
    $result->close();
}
?>