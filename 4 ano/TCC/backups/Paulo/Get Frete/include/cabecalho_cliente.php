...
$notifications = listNotifications(1, $_SESSION['logado']);

if (isset($_POST['action'])) {
  $action = $_POST['action'];

  if ($action == 'see') {
      seeNotifications(1, $_SESSION['logado']);
  } else {
      deleteNotifications(1, $_SESSION['logado']);
  }
}
...
<head>
    ...
    <!-- CSS Notificação de Proposta -->
    <link rel="stylesheet" href="../assets/css/proposal_style.css?v=<?php echo time(); ?>">
</head>
<body>
  <header>
    ...
    <div class="navbar-nav navbar-links">
      <a class="nav-item nav-link position_notifications" data-bs-toggle="modal" data-bs-target="#notificacoes">
        ...
        <!-- Nova Proposta -->
        <?php
          if(checkNotifications(1, $_SESSION['logado'])) {
    ...
    <div class="modal-content">
        ...
        <?php if (!empty($notifications)) : ?>
          <section class="notifications">
            ...
            <?php foreach ($notifications as $notification) : ?>
              <a href="../cliente/perfil.php?id=<?php echo $notification['id']  ?>#new_proposals" class="notification <?php echo $value = $notification["lido"] != 0 ? 'read': '' ;  ?>">
                <img src="../assets/img/caixa_verde.png" alt="Caixa">
                <p><?php echo $notification['motorista'].' te fez uma proposta!' ?></p>
              </a>
      ...
      <?php if (!empty($notifications)) : ?>
        <div class="modal_footer footer">
          <form action="../include/cabecalho_cliente.php" method="POST">
