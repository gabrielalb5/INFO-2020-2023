...
function listarPedidos_motorista($cidade,$uf){
    $conexao = obterConexao();
    $lista_pedidos = [];
    $sql = "SELECT p.id_p, p.descricao, p.origem, p.destino, p.data_entrega, p.horario, p.valor, p.status, cp.nome as 'categoria' FROM pedido p
    INNER JOIN categoria_pedido cp ON p.categoria = cp.id_cp
    WHERE p.origem LIKE '%$cidade%$uf%' ORDER BY p.data_pedido";
    $stmt = $conexao->prepare($sql);
    $stmt->execute();
    $resultado = $stmt->get_result();
    while($pedido = mysqli_fetch_assoc($resultado)){
