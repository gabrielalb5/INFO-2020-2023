...
function notificacao($pedido,$notificacao){
    ...
    switch($notificacao){
        case "recebe_proposta":
            ...
            $msg = 'Seu pedido "'.$descricao.'" recebeu uma nova proposta.';
            break;
        case "escolhe_proposta":
            $usuario = $pedido["motorista"];
            $msg = 'Sua proposta para "'.$descricao.'" foi escolhida.';
            break;
        case "inicio_entrega":
            $usuario = $pedido["cliente"];
...
function getAllNotificacoesJSON() {
    $resultado = listar_notificacoes();
    $lista_notificacoes = $resultado['notificacoes'];
    echo json_encode($lista_notificacoes);
}
if (isset($_GET['action']) && $_GET['action'] === 'get_all_notificacoes_json') {
    getAllNotificacoesJSON();
    exit; // Certifique-se de sair após a saída JSON
}
?>