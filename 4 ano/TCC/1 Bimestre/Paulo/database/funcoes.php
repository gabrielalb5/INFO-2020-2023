...
function cadastrarUsuario() {
    ...
    if (mysqli_num_rows($resultado)>0){
        ...
        if ($tipo_usuario == "cliente") {
            $senha_md5 = md5($usuario["senha"]);
            $sql = "INSERT INTO cliente (nome, sobrenome, nome_social, email, senha, cpf, telefone, uf, cidade) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conexao->prepare($sql);
            $stmt->bind_param("sssssssss", $usuario["nome"], $usuario["sobrenome"], $usuario["nome_social"], $usuario["email"], $senha_md5, $usuario["cpf"], $usuario["telefone"], $usuario["uf"], $usuario["cidade"]);
            $stmt->execute();
            $stmt->close();
...
//TESTE CADASTRO GENÉRICO DE IMAGENS DE PERFIL
function ImagemUsuario() {
    $imagem = $_SESSION["perfil"];
    $email = $_SESSION["email"];
    $pasta = $_SESSION["tipo_usuario"] == "cliente" ? "../assets/arquivos/clientes/" : "../assets/arquivos/motoristas/perfil/";
    ...
    if($deu_certo){
        $conexao = obterConexao();
        $sql = "INSERT INTO imagens_". $_SESSION['tipo_usuario'] ." (nome, path, email, extensao) VALUES (?,?,?,?)";
        $stmt = $conexao->prepare($sql);
    ...
    if($opcao_login=="motorista"){
        ...
        if(mysqli_num_rows($resultado)>0){
            ...
            if(mysqli_num_rows($resultado)>0){
                ...
                $_SESSION["tipo"] = $opcao_login;
                header("Location:../motorista/inicio_motorista.php");
            }else{
